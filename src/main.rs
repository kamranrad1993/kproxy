...
use kproxy::{Entry, StdioEntry, StdioStep, StepStatic};
...
fn main() {
    ...
    match result {
        Ok(cli_parsed) => {
            if cli_parsed.arguments.contains("Version") {
                let version_text = version(&cli_spec);
                println!("{}", version_text);
                exit(0);
            } else if cli_parsed.arguments.contains("Help") {
...
fn _main() {
    ...
    // let data: Vec<&str> = Vec::from_iter(env::args().map(|x| {x.as_ref()}));
    let args = Vec::from_iter(env::args());
    let args = args
        .iter()
        .skip(1)
...
//     arguments.add_option('e', "entry", "1", Some("Entry  Type"));

//     arguments.add_option('s', "step", "1 1 1", Some("Add Step"));
